...
private extension PolylineNode {
    ...
    struct Constants {
        static let defaultBuilder: BoxBuilder = { (distance) -> SCNBox in
            ...
            box.firstMaterial?.diffuse.contents = UIColor(red: 255.0/255.0, green: 2555.0/255.0, blue: 0.0/255.0, alpha: 1.0)
            return box
        }
    ...
    func contructNodes() {
        ...
        for i in 0 ..< polyline.pointCount - 1 {
            ...
            // connect each segment of line  from currentLoction to nextLocation
            boxNode.eulerAngles.y = Float(bearing).degreesToRadians
            // if the map obtain all the altitude data the slope can be created as below code
            //boxNode.eulerAngles.x = -1
